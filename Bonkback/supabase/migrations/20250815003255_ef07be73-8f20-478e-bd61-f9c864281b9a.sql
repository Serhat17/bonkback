...
-- Create encrypted storage for derivation paths (using built-in functions only)
...
AS $$
BEGIN
  -- Simple obfuscation using built-in functions (not cryptographically secure, but prevents casual access)
  -- In production, enable pgcrypto extension for proper encryption
  RETURN encode(convert_to(user_id::text || '::' || path, 'UTF8'), 'base64');
END;
$$;
...
BEGIN
  ...
  -- Simple base64 decode and extract path
  decrypted_text := convert_from(decode(encrypted_path, 'base64'), 'UTF8');
  path_part := split_part(decrypted_text, '::', 2);
  
  -- Verify the user_id matches
  IF split_part(decrypted_text, '::', 1) != user_id::text THEN
    RAISE EXCEPTION 'Key derivation data integrity check failed';
  END IF;
...
SET encrypted_derivation_path = public.encrypt_derivation_path(
  COALESCE(derivation_path, 'm/44''/501''/' || user_id::text || '''/' || version || ''''), 
  user_id
)
WHERE encrypted_derivation_path IS NULL;
