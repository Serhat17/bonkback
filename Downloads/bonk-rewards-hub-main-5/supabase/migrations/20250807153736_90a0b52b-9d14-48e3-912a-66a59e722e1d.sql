...
-- Phase 1: Production Hardening Migration (Fixed)
-- ==========================================
...
CREATE INDEX IF NOT EXISTS idx_cashback_transactions_offer_id ON public.cashback_transactions(offer_id);
CREATE INDEX IF NOT EXISTS idx_cashback_transactions_status ON public.cashback_transactions(status);
CREATE INDEX IF NOT EXISTS idx_cashback_transactions_created_at ON public.cashback_transactions(created_at);

CREATE INDEX IF NOT EXISTS idx_bonk_transfers_user_id ON public.bonk_transfers(user_id);
CREATE INDEX IF NOT EXISTS idx_bonk_transfers_status ON public.bonk_transfers(status);
CREATE INDEX IF NOT EXISTS idx_bonk_transfers_created_at ON public.bonk_transfers(created_at);
...
CREATE INDEX IF NOT EXISTS idx_cashback_offers_status ON public.cashback_offers(status);
CREATE INDEX IF NOT EXISTS idx_cashback_offers_merchant_name ON public.cashback_offers(merchant_name);

-- 2. ADD VALIDATION CONSTRAINTS
-- ==========================================

-- Email format validation (allow null)
...
-- First, ensure user_id is unique in profiles table
ALTER TABLE public.profiles 
...
BEGIN
  ...
  IF current_count >= _max_count THEN
    RETURN jsonb_build_object(
      ...
      'reset_at', window_start_time + (INTERVAL '1 minute' * _window_minutes)
    );
  END IF;
