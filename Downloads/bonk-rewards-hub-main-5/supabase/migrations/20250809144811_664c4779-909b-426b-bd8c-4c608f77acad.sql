-- Phase 2: RLS Policy Hardening - Fix naming conflicts
-- Remove complex nested policies and add explicit role guards

-- Drop all existing profile update policies to avoid conflicts
DROP POLICY IF EXISTS "Users can update their own profile" ON public.profiles;
DROP POLICY IF EXISTS "Users can update their own profile (except role)" ON public.profiles;

-- Create simplified profile update policy (without role restrictions in RLS)
