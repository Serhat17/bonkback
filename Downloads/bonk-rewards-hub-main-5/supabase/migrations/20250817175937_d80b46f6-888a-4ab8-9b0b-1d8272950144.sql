-- Update referral demo functions to actually credit BONK tokens to user balances
...
DECLARE
  ...
  v_referred_reward numeric := 333333;
BEGIN
  ...
  -- Actually credit BONK tokens for demo testing
  BEGIN
    -- Update referrer's balance (give them referral reward)
    UPDATE public.profiles 
    SET 
      bonk_balance = bonk_balance + v_referrer_reward,
      total_earned = total_earned + v_referrer_reward,
      updated_at = now()
    WHERE user_id = v_referrer_profile.user_id;

    -- Update current user's balance (give them welcome bonus)
    UPDATE public.profiles 
    SET 
      bonk_balance = bonk_balance + v_referred_reward,
      total_earned = total_earned + v_referred_reward,
      updated_at = now()
    WHERE user_id = v_current_user_id;

    -- Create demo referral payout records for audit trail
    INSERT INTO public.referral_payouts (
      referrer_id, referred_user_id, beneficiary_id, amount, status, 
      required_threshold, unlocked_at, created_at, updated_at
    ) VALUES
      -- Referrer gets reward
      (
        v_referrer_profile.user_id, v_current_user_id, v_referrer_profile.user_id,
        v_referrer_reward, 'completed', 0, now(), now(), now()
      ),
      -- Referred user gets welcome bonus
      (
        v_referrer_profile.user_id, v_current_user_id, v_current_user_id,
        v_referred_reward, 'completed', 0, now(), now(), now()
      );
    ...
    INSERT INTO public.security_audit_log (
      ...
      jsonb_build_object(
        ...
        'credited_rewards', jsonb_build_object(
    ...
    RETURN jsonb_build_object(
      'success', true,
      'message', 'Demo referral test successful! BONK rewards have been credited to both accounts.',
      'demo_data', jsonb_build_object(
        'referral_code', p_referral_code,
        'referrer', jsonb_build_object(
          'name', v_referrer_profile.full_name,
          'previous_balance', v_referrer_profile.bonk_balance,
          'credited_amount', v_referrer_reward,
          'new_balance', v_referrer_profile.bonk_balance + v_referrer_reward
        ),
        'referred_user', jsonb_build_object(
          'name', v_current_profile.full_name,
          'previous_balance', v_current_profile.bonk_balance,
          'credited_amount', v_referred_reward,
          'new_balance', v_current_profile.bonk_balance + v_referred_reward
        ),
        'total_demo_value', v_referrer_reward + v_referred_reward,
        'usd_equivalent', (v_referrer_reward + v_referred_reward) * 0.000025
      )
    );
  END;
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN jsonb_build_object(
    'success', false,
    'error', 'Demo test failed: ' || SQLERRM
  );
END;
$$;

-- Update the simulate referral signup demo to actually credit BONK
...
BEGIN
  ...
  -- Actually credit BONK tokens for the demo scenario
  BEGIN
    ...
    -- Update current user's balance (simulate new user welcome bonus)
    UPDATE public.profiles 
    SET 
      bonk_balance = bonk_balance + v_new_user_reward,
      total_earned = total_earned + v_new_user_reward,
      updated_at = now()
    WHERE user_id = v_current_user_id;
    ...
    RETURN jsonb_build_object(
      'success', true,
      'message', format('Demo: %s completed successfully! BONK tokens credited to both accounts.', p_scenario_name),
      'simulation_results', jsonb_build_object(
        ...
        'referrer', jsonb_build_object(
          'name', v_referrer_profile.full_name,
          'previous_balance', v_referrer_profile.bonk_balance,
          'credited_reward', v_referrer_reward,
          'new_balance', v_referrer_profile.bonk_balance + v_referrer_reward
        ),
        'new_user', jsonb_build_object(
          ...
          'new_balance', v_current_profile.bonk_balance + v_new_user_reward,
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', false,
    'error', 'Demo simulation failed: ' || SQLERRM
  );
END;
$$;